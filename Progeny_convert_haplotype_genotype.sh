...
echo "Correct usage: SCRIPT.sh [PLINK || HAPLOVIEW || HAPLOTYPE] genotype-file marker-file output_prefix [optional: individual-filter-proportion]"
echo "CHOOSE one mode: REFORMAT from Progeny to PLINK (Plink1.9 ped/map) or HAPLOTYPE using Shapeit (output is haps/sample combined with your phenotype data)"
...
if [ "$1" == "PLINK" ]; then
	...
	echo "Correct usage: SCRIPT.sh [PLINK || HAPLOVIEW || HAPLOTYPE] genotype-file marker-file output_prefix [optional: filter-proportion]"
	echo -e "\n"
	echo "Example usage: bash Progeny_haplotype_genotype.sh REFORMAT progeny_genotypes.txt marker_file.txt Omykiss_2018 0.9"
	echo -e "\n"
    exit 1
...
###write R file (parse_shapeit_output.R) to convert ShapeIt output to several more usable formats
...
echo "######writing fasta" >> parse_shapeit_output.R
echo "nucleotide_haplotypes<-read.delim(\"haplotypes.txt\",colClasses = c(\"factor\"))" >> parse_shapeit_output.R
echo "is.odd <- function(x) x %% 2 != 0" >> parse_shapeit_output.R
echo "fasta<-NULL" >> parse_shapeit_output.R
echo "for (i in 1:nrow(nucleotide_haplotypes)) {" >> parse_shapeit_output.R
echo "  if (is.odd(i)) {" >> parse_shapeit_output.R
echo "    fasta<-rbind(fasta,paste(\">\",as.character(nucleotide_haplotypes\$indiv[i]),\"A\",sep=\"\"))  " >> parse_shapeit_output.R
echo "  } else {" >> parse_shapeit_output.R
echo "    fasta<-rbind(fasta,paste(\">\",as.character(nucleotide_haplotypes\$indiv[i]),\"B\",sep=\"\"))  " >> parse_shapeit_output.R
echo "  }" >> parse_shapeit_output.R
echo "  fasta<-rbind(fasta,as.character(nucleotide_haplotypes\$MLhap[i]))" >> parse_shapeit_output.R
echo "}" >> parse_shapeit_output.R
echo "write.table(fasta,file=\"haplotypes.fasta\",append=FALSE,quote=FALSE,sep=\"\t\",col.names=F,row.names=F)" >> parse_shapeit_output.R
